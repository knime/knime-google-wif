...
public class GoogleWIFSettings implements DefaultNodeSettings {
    ...
    /**
     * The WIF configuration file.
     */
    @Widget(title = "WIF configuration file", description = "Google workload identity federation configuration file")
    @FileReaderWidget(fileExtensions = {"json"})
    public FileSelection m_configFile = new FileSelection();
    ...
    void validate(final PortObjectSpec[] inSpecs) throws InvalidSettingsException {
        if (StringUtils.isEmpty(m_configFile.getFSLocation().getPath())) {
            throw new InvalidSettingsException("Configuration file path not specified");
        }
    }
